...
def load_graph(view,graphname):
    ...
    adj=np.loadtxt('graph/%s-%s.txt'%(graphname,view),dtype='float32')
    num=int(adj.max()+1)
...
class Features(Dataset):
    ...
    def __init__(self,view,graphname):
        ...
        if graphname=="handwritten-5view":
            self.x=io.loadmat('%s/%s.mat'%(path,graphname))['X'][0][view]
            self.y=io.loadmat('%s/%s.mat'%(path,graphname))['Y'].squeeze()
