...
import paddle
import numpy as np
from zhouhai.plot import plot_cluster_result
...
def eva_pre_train(graphname,pre_train_flag):
    """
    评估预训练后的数据
    """
    dataLoader = MultiViewDataLoader(graphname)
...
def plot_train_cluster(graphname,pre_train_flag):

    xs_acc,xs_nmi,_,_=eva_pre_train(graphname,pre_train_flag)
    ...
    state_dict=paddle.load("zhouhai/results_train/%s.pkl"%graphname)
    model.set_state_dict(state_dict)
    inputs,targets=dataLoader.xs,dataLoader.y
    adjs=[]
...
def plot_root_data_cluster(graphname):
    ...
    for i,x in enumerate(inputs):
        ...
        plot_cluster_result(x,targets,'%s-原始数据聚类结果-%s'%(graphname,i))
